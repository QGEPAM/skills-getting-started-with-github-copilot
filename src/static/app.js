document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchActivities() {
    try {
      ...
      // Clear current activities and dropdown (avoid duplicates)
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        ? `<div class="participants-list">${details.participants
            .map(
              email =>
                `<div class="participant-row"><span>${email}</span><button class="remove-btn" data-activity="${encodeURIComponent(
                  name
                )}" data-email="${encodeURIComponent(email)}">✕</button></div>`
            )
            .join("")}</div>`
